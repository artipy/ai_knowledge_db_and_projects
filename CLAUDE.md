...
- **OpenAI API**: Python integration, API usage, prompt engineering techniques, text processing, chat roles (system/user/assistant), multi-turn conversations with context preservation, structured JSON responses, error handling (AuthenticationError, RateLimitError, BadRequestError), request batching, retry logic with tenacity, token counting with tiktoken, production-ready API wrappers, function calling (tools parameter, structured data extraction, parallel function calls, tool_choice, external API integration), production best practices (content moderation with safe/unsafe classification models, guardrails via system prompts, adversarial testing, API key security, user identification with UUID), embeddings applications (enriched embeddings combining multiple features, semantic search with find_n_closest, recommendation systems with single-item and history-based mean embedding aggregation, zero-shot classification with class description embeddings), vector databases with ChromaDB (PersistentClient, collections, add/get/update/upsert/delete, querying with query_texts, metadata filtering with where/$eq/$gt/$lt/$ne/$and/$or operators, cost estimation with tiktoken), vector databases with Pinecone (serverless vs pod-based indexes, Pinecone client, ServerlessSpec, create_index/list_indexes/delete_index, index.describe_index_stats, namespaces, upsert with metadata, query with top_k/filter/$eq/$ne, include_metadata, organization roles), Pinecone vector manipulation (fetch by ID/namespace, Read Units, query with top_k/include_values, distance metrics cosine/euclidean/dotproduct, metadata filtering with $eq/$gt/$ne/combined filters, update values/set_metadata/combined, delete by ID/filter/delete_all namespace)
- **Prompt Engineering**: Best practices for creating effective prompts, action verbs, structured prompts, formatted outputs (tables, lists, custom formats), conditional logic in prompts, iterative prompt development. Advanced strategies including Zero-Shot/One-Shot/Few-Shot prompting, Multi-Step Prompting, Chain-of-Thought (CoT), Self-Consistency prompting, and iterative refinement techniques. Business applications covering text summarization and extension, text transformation (translation, tone adjustment, writing improvement), text analysis (classification, entity extraction), and code generation and explanation. Chatbot development covering role definition, behavior configuration (tone, length, structure), setting boundaries and constraints, role-playing prompts, and incorporating external context.
- **Hugging Face**: Pipeline API for NLP tasks (text classification including sentiment analysis, grammar checking, QNLI, Zero-Shot classification), text summarization (extractive and abstractive), Auto-classes and tokenizers for fine-grained model control, Document QA with PDF support via pypdf
- **LLMOps**: Overview of LLMOps vs MLOps, LLM application lifecycle (Ideation, Development, Operation phases). Ideation phase: data sourcing assessment (relevance, availability, standardization), base model selection (proprietary vs open-source trade-offs), model selection factors (performance, characteristics, practical considerations). Development phase: Prompt Engineering (prompt structure, parameters, prompt management, templates), Chains & Agents (deterministic vs adaptive architectures), RAG (retrieval-augmented generation with vector databases and embeddings) vs Fine-tuning (Supervised FT, RLHF, catastrophic forgetting), Testing (statistical, LLM-as-a-Judge, categorical evaluation)
...
- **Practical Exercises**: 12 assignments covering OpenAI API Text Processing (shot prompting, temperature control, cost calculation), Chat Roles and Conversations (multi-turn dialogs, context preservation), Prompt Engineering Best Practices (structured prompts, formatting, conditional logic), Advanced Prompt Engineering Strategies (Zero/One/Few-Shot, Multi-Step, Chain-of-Thought, Self-Consistency, iterative refinement), Business Applications Prompt Engineering (text summarization/extension, text transformation, text analysis with multi-class classification, entity extraction with few-shot learning, code generation and explanation), Chatbot Development (role definition, behavior configuration, external context integration, multi-lingual support), Structuring End-to-End Applications (structured JSON responses, error handling, request batching, retry logic, token counting), Function Calling (function definition, structured data extraction, parallel function calls, tool_choice, external API integration), Working with Embeddings (embeddings API, batch processing, cosine similarity, semantic search, nearest-neighbor classification, t-SNE visualization, RAG search component), and Embeddings Applications (enriched embeddings, find_n_closest semantic search, single-item and history-based recommendations with mean embedding aggregation, zero-shot classification with label vs description comparison). Exercises 01-12 include student solutions and comprehensive reviews with detailed feedback, error analysis, and recommendations for improvement. Exercise 11 covers Vector Databases with ChromaDB (PersistentClient, collections, add/upsert/update/delete, query with metadata filtering, $and/$or operators, cost estimation). Exercise 12 covers Vector Databases with Pinecone (serverless index creation, upsert with metadata, query with top_k/filter, namespace management, idempotency, recommendations with postprocessing). Exercise 13 covers Pinecone Vector Manipulation (fetch by ID/namespace, query with include_values, distance metrics comparison, metadata filtering with $eq/$gt/$ne/combined, update vectors/metadata/both, delete by ID/filter/namespace, CRUD with namespaces).
